...
{
    private readonly IArrangedElement? _controlToLayout;
    private readonly bool _resumeLayout;

#if DEBUG
    private readonly int _layoutSuspendCount;
#endif
    public LayoutTransaction(IArrangedElement? controlToLayout, IArrangedElement controlCausingLayout, string? property)
        : this(controlToLayout, controlCausingLayout, property, true)
    {
    }

    public LayoutTransaction(IArrangedElement? controlToLayout, IArrangedElement controlCausingLayout, string? property, bool resumeLayout)
    {
        ...
        if (_controlToLayout is not null)
        {
            ...
            if (resumeLayout)
            {
                _controlToLayout.PerformLayout(controlCausingLayout, property);
            }
        }
    }
    ...
    public static IDisposable CreateTransactionIf(bool condition, IArrangedElement? controlToLayout, IArrangedElement elementCausingLayout, string? property)
    {
        if (condition)
        {
