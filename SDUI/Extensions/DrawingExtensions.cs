...
{
    private static readonly ContentAlignment anyRight =
    ...
    private static readonly ContentAlignment anyCenter =
    ...
    public static GraphicsPath CreateFanPath(this Graphics g, Point center, float d1, float d2, float startAngle,
        float sweepAngle)
    {
        return center.CreateFanPath(d1, d2, startAngle, sweepAngle);
    }

    public static GraphicsPath CreateFanPath(this Graphics g, PointF center, float d1, float d2, float startAngle,
        float sweepAngle)
    {
        return center.CreateFanPath(d1, d2, startAngle, sweepAngle);
    }
    ...
    {
        graphics.DrawShadow(rect.ToRectangleF(), size, radius, color);
    }

    public static void DrawShadow(this Graphics graphics, RectangleF rect, float size, int radius,
        Color color = default)
    {
        if (size <= 0)
            return;
    ...
    public static StringFormat StringFormatForAlignment(ContentAlignment align)
    {
        return new StringFormat
            { Alignment = TranslateAlignment(align), LineAlignment = TranslateLineAlignment(align) };
    }

    public static StringFormat CreateStringFormat(this Control ctl, ContentAlignment textAlign, bool showEllipsis,
        bool useMnemonic)
    {
        var format = StringFormatForAlignment(textAlign);
        if (ctl.RightToLeft == RightToLeft.Yes) format.FormatFlags |= StringFormatFlags.DirectionRightToLeft;
        if (showEllipsis)
        {
        ...
        if (ctl.AutoSize) format.FormatFlags |= StringFormatFlags.MeasureTrailingSpaces;
        return format;
    }

    public static void DrawString(this Control control, string text, Graphics graphics,
        ContentAlignment contentAlignment, bool showEllipsis = false, bool useMnemonic = false)
    {
    ...
    public static void DrawString(this Control control, Graphics graphics, ContentAlignment contentAlignment,
        bool showEllipsis = false, bool useMnemonic = false)
    {
        graphics.TextRenderingHint = TextRenderingHint.SystemDefault;
        using var textFormat = control.CreateStringFormat(contentAlignment, showEllipsis, useMnemonic);
        using var textBrush = new SolidBrush(control.ForeColor);
    ...
    public static void DrawString(this Control control, Graphics graphics, ContentAlignment contentAlignment,
        Color color, bool showEllipsis = false, bool useMnemonic = false)
    {
        graphics.TextRenderingHint = TextRenderingHint.SystemDefault;
        using var textFormat = control.CreateStringFormat(contentAlignment, showEllipsis, useMnemonic);
    ...
    public static void DrawString(this Control control, Graphics graphics, ContentAlignment contentAlignment,
        Color color, RectangleF rectangle, bool showEllipsis = false, bool useMnemonic = false)
    {
        graphics.TextRenderingHint = TextRenderingHint.SystemDefault;
        using var textFormat = control.CreateStringFormat(contentAlignment, showEllipsis, useMnemonic);
        using var textBrush = new SolidBrush(color);
    ...
    public static void DrawString(this Control control, Graphics graphics, string text, Color color,
