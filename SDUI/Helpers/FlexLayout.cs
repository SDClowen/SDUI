...
namespace SDUI.Helpers
{
    ...
    public void PerformLayout(UIElementBase container, Rectangle clientArea)
    {
        var controls = container.Controls
            ...
            .Where(c => c.Visible && c.Dock == System.Windows.Forms.DockStyle.None)
            .ToList();
    ...
    private void LayoutSingleLine(List<UIElementBase> controls, Rectangle area, bool isHorizontal)
    {
        ...
        int[] positions = CalculateMainAxisPositions(
            controls.Count,
            totalSize,
        ...
        for (int i = 0; i < controls.Count; i++)
        {
            ...
            int crossPos = CalculateCrossAxisPosition(
    ...
    private void LayoutMultiLine(List<UIElementBase> controls, Rectangle area, bool isHorizontal)
    {
        ...
        int maxLineSize = isHorizontal ? area.Width : area.Height;

        foreach (var control in controls)
        {
            int controlSize = isHorizontal ? control.Size.Width : control.Size.Height;
            int requiredSize = currentLineSize + controlSize + (currentLine.Count > 0 ? Gap : 0);
        ...
        foreach (var line in lines)
        {
            ...
            int[] positions = CalculateMainAxisPositions(line.Count, lineMainSize, maxLineSize);
    ...
    private int[] CalculateMainAxisPositions(int count, int totalSize, int availableSize)
    {
        ...
        switch (JustifyContent)
        {
            ...
            case JustifyContent.FlexEnd:
                int offset = availableSize - totalSize;
                pos = offset;
                for (int i = 0; i < count; i++)
                {
                    positions[i] = pos;
            ...
            case JustifyContent.SpaceBetween:
                ...
                {
                    int spacing = (availableSize - totalSize + Gap * (count - 1)) / (count - 1);
                    pos = 0;
                    for (int i = 0; i < count; i++)
                    {
                        positions[i] = pos;
            ...
            case JustifyContent.SpaceAround:
                int space = (availableSize - totalSize + Gap * (count - 1)) / count;
                pos = space / 2;
                for (int i = 0; i < count; i++)
                {
                    positions[i] = pos;
