...
public static class DpiHelper
{
    ...
    public static int GetSystemDpi()
    {
        ...
        try
        {
            var dpiX = GetDeviceCaps(screenDC, LOGPIXELSX);
            var dpiY = GetDeviceCaps(screenDC, LOGPIXELSY);
    ...
    public static int GetDpiForWindowInternal(IntPtr handle)
    {
        ...
        if (monitor != IntPtr.Zero)
            try
            {
                if (GetDpiForMonitor(monitor, MonitorDpiType.EffectiveDpi, out var dpiX, out var _) == 0)
                    return (int)dpiX;
            }
            catch (DllNotFoundException)
            {
