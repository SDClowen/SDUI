...
namespace SDUI.Controls
{
    ...
    public class StatusStrip : MenuStrip
    {
        ...
        private Padding _gripMargin = new Padding(3);
        ...
        [Category("Layout")]
        public float ItemPadding { get => _itemPadding; set { if (_itemPadding == value) return; _itemPadding = value; Invalidate(); } }

        [Category("Layout")]
        [DefaultValue(true)]
        public bool SizingGrip { get => _sizingGrip; set { if (_sizingGrip == value) return; _sizingGrip = value; Invalidate(); } }

        [Category("Layout")]
        public Padding GripMargin { get => _gripMargin; set { if (_gripMargin == value) return; _gripMargin = value; Invalidate(); } }

        [Category("Layout")]
        [DefaultValue(ToolStripLayoutStyle.HorizontalStackWithOverflow)]
        public ToolStripLayoutStyle LayoutStyle { get => _layoutStyle; set { if (_layoutStyle == value) return; _layoutStyle = value; Invalidate(); } }
        ...
        public override void OnPaint(SKPaintSurfaceEventArgs e)
        {
            ...
            if (SizingGrip)
            {
                ...
                var dotSize = 3f;
                var gap = 4f;
                float startX = bounds.Width - GripMargin.Right - 4f;
                float startY = bounds.Height - GripMargin.Bottom - 4f;

                for (int row = 0; row < 3; row++)
                {
                    for (int col = 0; col <= row; col++)
                    {
                        float x = startX - (col * gap) - (row * 1f);
                        float y = startY - (row * gap) + (col * 0f);
