...
public class TreeView : UIElementBase
{
    ...
    [Category("Behavior")] public bool FullRowSelect { get; set; } = true;

    [Category("Behavior")] public bool HideSelection { get; set; } = false;
    ...
    {
        var tf = FontManager.GetSKTypeface(font);
        return new SKFont(tf, GetSkTextSize(font))
    ...
    private float DrawNode(SKCanvas canvas, TreeNode node, int depth, float y, SKFont font, SKPaint textPaint)
    {
        ...
        var fm = font.Metrics;
        var capHeight = Math.Abs(fm.CapHeight) > 0 ? Math.Abs(fm.CapHeight) : font.Size;
        // Clamp toggle size to fit the row and fonts
        var toggleSize = Math.Min(_rowHeight - 4, Math.Max(8f, capHeight * 0.9f));
        if (hasChildren)
        {
            ...
            var progress = (float)(_nodeAnimations.TryGetValue(node, out var na) ? na.GetProgress() :
                node.Expanded ? 1.0 : 0.0);
            ...
            var width = toggleSize * 0.6f;
            var halfW = width / 2f;
            var halfH = toggleSize / 2f;
        ...
        // text
        var textX = x + (hasChildren ? toggleSize + 6 : 0);
        var text = node.Text;
        ...
        foreach (var kv in _toggleBounds)
            if (kv.Value.Contains(p))
            {
                ...
                if (!n.Expanded)
                {
                    ...
                    {
                        AnimationType = AnimationType.EaseInOut, Increment = 0.12, Singular = true,
                        InterruptAnimation = true
                    };
                    am.OnAnimationProgress += _ => Invalidate();
                    am.OnAnimationFinished += _ =>
                    {
                        /* nothing special for expand */
                    };
                ...
                else
                {
                    ...
                    am.OnAnimationFinished += _ =>
                    {
                        _pendingCollapse.Remove(n);
                        n.Expanded = false;
                        _nodeAnimations.Remove(n);
                        Invalidate();
                    };
                    _nodeAnimations[n] = am;
        ...
        foreach (var kv in _nodeBounds)
            if (kv.Value.Contains(p))
            {
                ...
                if (e.Button == MouseButtons.Right)
                {
                    ...
                    NodeMouseRightClick?.Invoke(this,
                        new TreeNodeMouseClickEventArgs(node, e.Button, e.Clicks, e.Location));
                ...
                if (e.Clicks >= 2)
                {
                    ...
                    _lastSelectedNode = node;
                    SelectedNodesChanged?.Invoke(this, EventArgs.Empty);
                    NodeMouseDoubleClick?.Invoke(this,
                        new TreeNodeMouseClickEventArgs(node, e.Button, e.Clicks, e.Location));
                    NodeMouseClick?.Invoke(this, new TreeNodeMouseClickEventArgs(node, e.Button, e.Clicks, e.Location));
                    Invalidate();
                    return;
                ...
                else if ((ModifierKeys & Keys.Shift) == Keys.Shift && _lastSelectedNode != null)
                {
                    ...
                    var a = list.IndexOf(_lastSelectedNode);
                    var b = list.IndexOf(node);
                    if (a >= 0 && b >= 0)
                    {
                        var start = Math.Min(a, b);
                        var end = Math.Max(a, b);
                        _selectedNodes.Clear();
                        for (var i = start; i <= end; i++) _selectedNodes.Add(list[i]);
                        SelectedNodesChanged?.Invoke(this, EventArgs.Empty);
                    }
                    else
                    {
    ...
    private void MoveSelection(int delta)
    {
        ...
        var idx = SelectedNode == null ? -1 : list.IndexOf(SelectedNode);
